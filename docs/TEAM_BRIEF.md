...
- Model runtime: Mock, Ollama scaffold, MLX Gemma 4 presets, and private remote Gemma 4 presets configured by environment variable.
...
- Private remote presets are useful when the Mac GPU is already occupied. They keep the same GemmaLens backend/API path while routing generation through `REMOTE_GEMMA_BASE_URL`.
- The remote Q4 preset is preferred for fast functional testing when a quantized endpoint is available, leaving the fp16 endpoint for quality runs.
...
- Current active test runtime can be switched to a remote E2B or E4B preset for private CPU/edge testing.
...
- Internal A/B controls are no longer a learner-facing route. Stale `/experiments` links redirect to Guide, and controls remain limited to the demo-analysis path when needed for team experiments.
